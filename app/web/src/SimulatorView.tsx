...
import { draw } from "motsuyoku-sensor-core";
...
// シミュレーターモード: 条件変更のたびに確率をリアルタイム表示し、10連はすべて結果を見せる
// (研究モードのような途中停止はしない)。experiment_ui_flow_spec.md 4章。
export function SimulatorView({ datasets, dataset, onSelectDataset }: Props) {
  ...
  const {
    ...
    exportJSON,
    exportCSV,
  } = useSimulationHistory(dataset, target, theoreticalProbability);
    ...
    <>
      ...
      <main className="app-layout">
        ...
        <section className="app-layout__right">
          <RecordingStatusBar
            sessionTotalDraws={sessionTotalDraws}
